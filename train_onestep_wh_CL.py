...
if __name__ == '__main__':
    ...
    parser.add_argument('--out-file', type=str, default="ckpt_onestep_wh_per10_0", metavar='S',
                        help='Saved model name')
    parser.add_argument('--in-file', type=str, default="ckpt_onestep_wh_try3.12", metavar='S',
    ...
    train_ds = whCLDataset(seq_len=cfg.seq_len, ts=0.01, seed=42, perturb_percentage= 10)
    train_dl = DataLoader(train_ds, batch_size=cfg.batch_size)

    # if we work with a constant model we also validate with the same (thus same seed!)
    val_ds = whCLDataset(seq_len=cfg.seq_len, ts=0.01, seed=42, perturb_percentage= 10)
